...
Restart Claude Code. Run `/help` and you should see `/delegate-work`, `/tier-model`, `/scout-codegraph`, `/scratch-context`.

This is skills-only — no hooks, no MCP auto-wiring. The `scout-codegraph` skill requires manually wiring the CodeGraph MCP server (see its SKILL.md). The `scratch-context` skill works but you have to remember to read your notes manually. For shell-output trimming, install RTK separately (`cargo install rtk && rtk init -g`).
...
```bash
# CodeGraph — for structural code queries via MCP. Required for the
# auto-wired `codegraph` MCP server to start; the plugin still works
# without it (MCP just fails to start, logged but non-fatal).
...
**Recommended (separate install, ships its own Claude Code hook):**
...
Skills are now namespaced: `/te:delegate-work`, `/te:tier-model`, etc. The plugin's `SessionStart` hook injects `.agent-scratch/` notes automatically. CodeGraph MCP is auto-registered; check with `/mcp`.
...
[RTK](https://github.com/rtk-ai/rtk) sits underneath all of these as an optional companion: even when a command does run in the supervisor, RTK trims the output before the model sees it. Install separately; not part of this plugin.
...
skills/                      the four skills (work standalone or as plugin)
templates/
